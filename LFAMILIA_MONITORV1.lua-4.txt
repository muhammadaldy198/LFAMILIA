...
local function getMentionsForPlayer(playerName)
    local mentions = {}
    local seenDiscord = {}
    local target = tostring(playerName or ""):lower()

    for discordId, playerList in pairs(mappings) do
        if type(playerList) == "table" then
            for _, mappedPlayer in ipairs(playerList) do
                if tostring(mappedPlayer):lower() == target then
...
local function sendWebhook(data)
    ...
    local mention = getMentionsForPlayer(data.Player)
    local catchLabel = (variant and (tostring(variant) .. " ") or "") .. tostring(data.Fish)
    local assetLabel = assetInfo.asset_uri or "Asset private tidak tersedia"

    -- Bentuk embed disertakan bersama field mentah agar relay Python nanti dapat
    -- langsung meneruskannya ke Discord tanpa perlu menebak format ulang.
    local embed = {
        title = "🎣 " .. tostring(rarityName) .. " Catch Detected",
        description = tostring(data.Player) .. " obtained a " .. catchLabel
            .. " (" .. tostring(data.Weight) .. "kg).",
        color = embedColor,
        fields = {
            {name = "Player", value = tostring(data.Player), inline = true},
            {name = "Catch", value = catchLabel, inline = true},
            {name = "Rarity", value = tostring(rarityName), inline = true},
            {name = "Weight", value = tostring(data.Weight) .. " kg", inline = true},
            {name = "Chance", value = tostring(data.Chance or "unknown"), inline = true},
            {name = "Asset", value = assetLabel, inline = false},
        },
        footer = {text = "LFAMILIA Radar • Fish It"},
...
local function sendJoinLeaveWebhook(playerName, action)
    ...
    if not accepted then
        logError("Webhook join/leave gagal: " .. tostring(err or (statusCode and ("HTTP " .. statusCode) or "respons kosong")))
    end
end
-- =============================================================================
...
local normalMainSize = UDim2.new(0.92, 0, 0, 535)
...
headerStatus.Size = UDim2.new(0, 104, 0, 22)
headerStatus.Position = UDim2.new(1, -150, 0, 23)
...
tabbar.Position = UDim2.new(0, 12, 1, -57)
...
local function makePage(name)
    ...
    page.Name = name
    page.Position = UDim2.new(0, 14, 0, 76)
    page.Size = UDim2.new(1, -28, 1, -140)
...
local homeStatus = card(home, 92, THEME.Green)
...
local mappingText = Instance.new("TextLabel")
mappingText.Position = UDim2.fromOffset(10, 8)
mappingText.Size = UDim2.new(1, -20, 0, 110)
mappingText.AutomaticSize = Enum.AutomaticSize.Y
mappingText.BackgroundTransparency = 1
mappingText.Text = "Belum ada mapping."
mappingText.TextColor3 = THEME.Text
mappingText.TextSize = 10
mappingText.Font = Enum.Font.Code
mappingText.TextXAlignment = Enum.TextXAlignment.Left
mappingText.TextYAlignment = Enum.TextYAlignment.Top
mappingText.TextWrapped = true
mappingText.Parent = mappingScroll
...
local webhookInput = makeInput(webhookCard, 57, CONFIG.WEBHOOK_URL, "Catch relay URL (Discord webhook utama)")
local logWebhookInput = makeInput(webhookCard, 89, CONFIG.JOIN_LEAVE_URL, "Join/leave relay URL (opsional)")
...
local infoUI = text(webhookCard, "Mention hanya berasal dari mapping akun di tab Players.", 181, 9, THEME.Muted)

local previewCard = card(webhookPage, 303, THEME.Pink)
...
discordMock.Size = UDim2.new(1, -26, 0, 230)
...
previewBar.Size = UDim2.new(0, 4, 0, 151)
previewBar.BackgroundColor3 = THEME.Pink
previewBar.BorderSizePixel = 0
previewBar.Parent = discordMock
addCorner(previewBar, 2)

local previewEmbed = Instance.new("Frame")
previewEmbed.Position = UDim2.fromOffset(23, 61)
previewEmbed.Size = UDim2.new(1, -35, 0, 151)
...
local previewTitle = text(previewEmbed, "🎣 Legendary Catch Detected", 9, 12, THEME.Text, true)
previewTitle.Position = UDim2.fromOffset(12, 8)
previewTitle.Size = UDim2.new(1, -88, 0, 22)

local previewBody = text(previewEmbed, "Player obtained a fish.", 38, 10, Color3.fromRGB(219, 221, 225))
previewBody.Position = UDim2.fromOffset(12, 36)
previewBody.Size = UDim2.new(1, -88, 0, 31)
previewBody.TextWrapped = true

local previewMeta = text(previewEmbed, "Legendary  •  5.50 kg  •  1 in 6K", 77, 9, THEME.Orange, true)
previewMeta.Position = UDim2.fromOffset(12, 72)
previewMeta.Size = UDim2.new(1, -88, 0, 18)
...
previewImage.Position = UDim2.new(1, -72, 0, 15)
...
dialog.Size = UDim2.new(0.92, 0, 0, 455)
...
dropdownList.Size = UDim2.new(1, -36, 0, 247)
...
local mappingDialogStatus = text(dialog, "Pilih player dari daftar aktif.", 390, 9, THEME.Muted)
mappingDialogStatus.Position = UDim2.fromOffset(18, 391)
...
refreshMappings = function()
    ...
    for discordId, playerList in pairs(normalized) do
        ...
        lines[#lines + 1] = "<@" .. tostring(discordId) .. ">  ←  " .. table.concat(playerList, ", ")
    end

    table.sort(lines)
    mappingText.Text = #lines > 0 and table.concat(lines, "\n")
        or "Belum ada mapping. Klik tombol di bawah untuk menambahkan akun."
end
...
refreshWebhookPreview = function(data)
    ...
    previewMention.Text = mention ~= "" and mention or "No account mention"
    previewTitle.Text = "🎣 " .. rarityName .. " Catch Detected"
    previewBody.Text = playerName .. " obtained a " .. catchName .. " (" .. weightText .. ")."
    previewMeta.Text = rarityName .. "  •  " .. weightText .. "  •  " .. chanceText
    previewBar.BackgroundColor3 = Color3.fromRGB(
        tonumber(rarityRGB[1]) or 255,
        tonumber(rarityRGB[2]) or 255,
        tonumber(rarityRGB[3]) or 255
    )

    if assetInfo.asset_id then
        local assetStatus = assetInfo.thumbnail_url and "public thumbnail" or "private Fish It asset"
        previewAssetText.Text = "Asset ID: " .. assetInfo.asset_id .. "\n" .. assetStatus
    else
        previewAssetText.Text = "Asset ID: belum ditemukan"
    end
...
testWebhook.Activated:Connect(function()
    ...
    infoUI.Text = sent
        and "Preview dibuat dan dikirim ke relay catch."
...
min.Activated:Connect(function()
    ...
    main.Size = minimized and UDim2.fromOffset(210, 48) or normalMainSize
    header.Size = minimized and UDim2.new(1, -52, 0, 43) or UDim2.new(1, -62, 0, 58)
