...
local function getMentionsForPlayer(playerName, playerObject, webhookKind)
        ...
        if type(playerList) == "table" then
            for _, mappedPlayer in ipairs(playerList) do
                if playerNamesMatch(mappedPlayer, playerName, playerObject) then
                    ...
                    if id ~= "" then
                        local prefs = state.MappingWebhookPrefs[id]
                        if type(prefs) ~= "table" then
                            prefs = {fish = true, leave = true}
                            state.MappingWebhookPrefs[id] = prefs
                        end
                        if type(prefs.fish) ~= "boolean" then prefs.fish = true end
                        if type(prefs.leave) ~= "boolean" then prefs.leave = true end

                        local enabled = true
                        if webhookKind == "fish" or webhookKind == "leave" then
                            enabled = prefs[webhookKind] ~= false
                        end

                        if enabled then
                            hasEnabledMapping = true
                            if not seenDiscord[id] then
                                seenDiscord[id] = true
    ...
    table.sort(mentions)
    return table.concat(mentions, " "), hasMapping, hasEnabledMapping
end
...
local function sendWebhook(data)
    ...
    local mention, hasFishMapping, hasFishEnabled = getMentionsForPlayer(
        data.Player,
        Players:FindFirstChild(data.Player),
        "fish"
    )
...
local function sendJoinLeaveWebhook(playerOrName, action)
    ...
    local mention, hasLeaveMapping, hasLeaveEnabled = getMentionsForPlayer(
        playerName,
        playerObject,
        action == "LEFT" and "leave" or nil
    )
    if action == "LEFT" and hasLeaveMapping and not hasLeaveEnabled then
...
local mappingBox = card(logPage, 400, THEME.Purple)
text(mappingBox, "🔗 DISCORD ACCOUNT MAPPING", 10, 11, THEME.Purple, true)
local mappingHint = text(mappingBox, "Masukkan ID Discord, pilih player, lalu atur FISH / LEAVE per ID.", 35, 9, THEME.Muted)
...
mappingScroll.Size = UDim2.new(1, -26, 0, 252)
...
refreshMappings = function()
    ...
    for index, discordId in ipairs(discordIds) do
        ...
        label.Size = UDim2.new(1, -18, 0, 30)
        label.BackgroundTransparency = 1
        ...
        local fishButton = makeButton(
            ...
            "",
            UDim2.new(0, 9, 0, 43),
            UDim2.new(0.32, -6, 0, 30),
            THEME.Green
        )
        local leaveButton = makeButton(
            row,
            "",
            UDim2.new(0.32, 6, 0, 43),
            UDim2.new(0.32, -6, 0, 30),
            THEME.Green
        )
        local removeButton = makeButton(
            row,
            "DELETE",
            UDim2.new(0.64, 12, 0, 43),
            UDim2.new(0.36, -21, 0, 30),
            THEME.Red
        )

        local function updateMappingButtons()
            fishButton.Text = "FISH  " .. (prefs.fish and "ON" or "OFF")
            fishButton.BackgroundColor3 = prefs.fish and THEME.Green or THEME.Surface2
            fishButton.TextColor3 = prefs.fish and Color3.fromRGB(8, 20, 15) or THEME.Muted

            leaveButton.Text = "LEAVE  " .. (prefs.leave and "ON" or "OFF")
            leaveButton.BackgroundColor3 = prefs.leave and THEME.Green or THEME.Surface2
            leaveButton.TextColor3 = prefs.leave and Color3.fromRGB(8, 20, 15) or THEME.Muted
        end
        updateMappingButtons()

        fishButton.Activated:Connect(function()
            prefs.fish = not prefs.fish
            updateMappingButtons()
            mappingLogStatus.TextColor3 = THEME.Orange
            mappingLogStatus.Text = "FISH " .. (prefs.fish and "ON" or "OFF") .. " untuk <@" .. discordId .. ">. CONFIG → SAVE."
        end)

        leaveButton.Activated:Connect(function()
            prefs.leave = not prefs.leave
            updateMappingButtons()
            mappingLogStatus.TextColor3 = THEME.Orange
            mappingLogStatus.Text = "LEAVE " .. (prefs.leave and "ON" or "OFF") .. " untuk <@" .. discordId .. ">. CONFIG → SAVE."
        end)
...
mapSave.Activated:Connect(function()
    ...
    if type(state.MappingWebhookPrefs[discordId]) ~= "table" then
        state.MappingWebhookPrefs[discordId] = {fish = true, leave = true}
    end
    refreshMappings()
...
Players.PlayerAdded:Connect(function(player)
    ...
    pcall(updateServerStats)
    -- Webhook JOIN dinonaktifkan; log JOIN/UI tetap berjalan.
end)
