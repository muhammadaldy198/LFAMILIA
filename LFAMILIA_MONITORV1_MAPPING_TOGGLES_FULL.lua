...
local CONFIG = {
    ...
    -- Filter khusus Ruby dengan mutation Gemstone. OFF = perilaku normal.
    GEMSTONE_RUBY_FILTER = false,
    -- LEGACY: dipertahankan agar profile lama tetap kompatibel.
    -- Nilai ini tidak lagi dipakai untuk memfilter catch.
    MIN_WEIGHT = 0,
...
state.ensureMappingWebhookPrefs = function(discordId)
    discordId = normalizeDiscordId(discordId)
    if discordId == "" then
        return {Fish = true, Leave = true}
    end

    local prefs = state.MappingWebhookPrefs[discordId]
    if type(prefs) ~= "table" then
        prefs = {Fish = true, Leave = true}
        state.MappingWebhookPrefs[discordId] = prefs
    end
...
local function manualSave(slot)
    ...
    local data = {
        ...
        CONFIG = {
            ...
            GEMSTONE_RUBY_FILTER = CONFIG.GEMSTONE_RUBY_FILTER == true,
            -- Tetap disimpan sebagai 0 untuk kompatibilitas profile versi lama.
            MIN_WEIGHT = 0,
            WEBHOOK_USERNAME = CONFIG.WEBHOOK_USERNAME,
        },
        mappings = normalizeMappings(mappings),
        mappingWebhookPrefs = state.MappingWebhookPrefs,
    }
...
local function manualLoad(slot)
    ...
    if data.CONFIG then
        ...
        if type(data.CONFIG.MIN_WEIGHT) == "number" then
            -- Profile lama boleh masih menyimpan MIN_WEIGHT, tetapi filter berat
            -- sudah dihapus sehingga nilainya selalu dinonaktifkan.
            CONFIG.MIN_WEIGHT = 0
        end
        ...
        if type(data.CONFIG.GEMSTONE_RUBY_FILTER) == "boolean" then
            CONFIG.GEMSTONE_RUBY_FILTER = data.CONFIG.GEMSTONE_RUBY_FILTER
        end
    end
    ...
    table.clear(state.MappingWebhookPrefs)
    if type(data.mappingWebhookPrefs) == "table" then
        for rawDiscordId, rawPrefs in pairs(data.mappingWebhookPrefs) do
            ...
            if discordId ~= "" and type(rawPrefs) == "table" then
                state.MappingWebhookPrefs[discordId] = {
    ...
    -- Profile lama yang belum punya toggle otomatis tetap ON/ON.
    for discordId in pairs(mappings) do
        state.ensureMappingWebhookPrefs(discordId)
    end
...
local function getVariantFromName(text)
    ...
    for _, variantData in pairs(VariantDatabase) do
        if type(variantData) == "table" and variantData.Name then
            ...
            local variantLower = variantName:lower()
            local prefix = variantLower .. " "

            -- SHINY adalah status/cosmetic fish, bukan mutation.
            if variantLower ~= "shiny" and lowerText:sub(1, #prefix) == prefix then
...
local function getVariantByExactName(text)
    ...
    for _, variantData in pairs(VariantDatabase) do
        if type(variantData) == "table" and variantData.Name then
            local variantLower = tostring(variantData.Name):lower()
            if variantLower ~= "shiny" and variantLower == lowerText then
...
local function getVariantByColor(r, g, b)
    ...
    for _, variantData in pairs(VariantDatabase) do
        if type(variantData) == "table" and variantData.Name
            and tostring(variantData.Name):lower() ~= "shiny"
...
local function parseCatch(message)
    ...
    if not variantName and not isExactFishName then
        for index, span in ipairs(spans) do
            -- Lewati span player, SHINY, dan jangan memakai warna span fish sebagai variant.
            if index > 1 and span.Text:lower() ~= "shiny"
                and not span.Text:find("%([^)]-%)") then
...
local function getDatabaseFishNameWithoutSize(name)
    ...
    -- Mendukung kombinasi berantai: "Shiny X", "Big Shiny X",
    -- "Shiny Big X", dst. Nama yang ditampilkan tidak pernah diubah.
    for _ = 1, 6 do
        ...
        local _, realName = findFishExact(candidate)
        if realName then
            return realName
        end
    end
...
state.getMappingDeliveryState = function(playerName, playerObject, webhookKind)
        ...
        if type(playerList) == "table" then
            for _, mappedPlayer in ipairs(playerList) do
                if playerNamesMatch(mappedPlayer, playerName, playerObject) then
                    hasMapping = true
                    local prefs = state.ensureMappingWebhookPrefs(discordId)
                    if webhookKind == "Fish" then
                        hasEnabledMapping = prefs.Fish ~= false
                    elseif webhookKind == "Leave" then
                        hasEnabledMapping = prefs.Leave ~= false
                    else
        ...
        if type(playerList) == "table" then
            for _, mappedPlayer in ipairs(playerList) do
                if playerNamesMatch(mappedPlayer, playerName, playerObject) then
                    ...
                    local prefs = state.ensureMappingWebhookPrefs(id)
                    local enabled = true
                    if webhookKind == "Fish" then
                        enabled = prefs.Fish ~= false
                    elseif webhookKind == "Leave" then
                        enabled = prefs.Leave ~= false
                    end
...
local function sendWebhook(data)
    ...
    -- Ambil varian/mutasi lebih awal karena Gemstone Ruby adalah pengecualian
    -- khusus terhadap filter rarity Legendary.
    ...
    -- SPECIAL BYPASS: Gemstone Ruby tetap boleh terkirim walaupun filter
    -- Legendary dimatikan. Catch lain tetap wajib mengikuti filter rarity.
    -- Jadi Ruby mutation lain dan ikan Legendary lain TIDAK ikut lolos saat
    -- Legendary OFF.
    local _, resolvedFishNameForSpecial = resolveFish(data.Fish)
    local isGemstoneRuby = CONFIG.GEMSTONE_RUBY_FILTER == true
        and tostring(resolvedFishNameForSpecial or ""):lower() == "ruby"
        and trim(variant):lower() == "gemstone"

    local rarityAllowed = CONFIG.ALLOW_RARITY[rarityName] == true
        or CONFIG.ALLOW_RARITY[string.upper(rarityName)] == true

    if not rarityAllowed and not isGemstoneRuby then
        return false
    end

    -- Filter weight sudah dihapus. Weight tetap diparsing dan ditampilkan
    -- di webhook, tetapi tidak pernah dipakai untuk menolak catch.
    ...
    -- Toggle FISH berlaku hanya pada Discord ID yang memetakan player ini.
    local mappedPlayerObject = Players:FindFirstChild(data.Player)
    local hasFishMapping, fishEnabled = state.getMappingDeliveryState(data.Player, mappedPlayerObject, "Fish")
    if hasFishMapping and not fishEnabled then
        appendMonitorLog("⏸", "FISH webhook OFF untuk mapping: " .. tostring(data.Player))
        return false
    end

    -- Mention hanya berasal dari ID yang FISH-nya aktif.
...
local function sendJoinLeaveWebhook(playerOrName, action)
    ...
    if action == "LEFT" then
        local hasLeaveMapping, leaveEnabled = state.getMappingDeliveryState(playerName, playerObject, "Leave")
        if hasLeaveMapping and not leaveEnabled then
            appendMonitorLog("⏸", "LEAVE webhook OFF untuk mapping: " .. tostring(playerName))
            return
        end
    end
...
local THEME = {
    ...
    Divider = Color3.fromRGB(91, 105, 146),
}
...
tabStroke.Color = THEME.Divider
tabStroke.Transparency = 0.10
...
local function addStroke(object, color, transparency)
    ...
    -- Semua outline dibuat lebih solid; nilai lama yang terlalu transparan
    -- otomatis dibatasi agar garis tetap jelas.
    local requestedTransparency = tonumber(transparency) or 0.08
    stroke.Transparency = math.min(requestedTransparency, 0.20)
...
local function card(parent, height, accent)
    ...
    frame.Parent = parent
    addCorner(frame, 10)
    addStroke(frame, accent or THEME.Divider, 0.14)

    -- Garis aksen horizontal solid di bagian atas setiap card.
    local topLine = Instance.new("Frame")
    topLine.Size = UDim2.new(1, -22, 0, 2)
    topLine.Position = UDim2.fromOffset(11, 0)
    topLine.BackgroundColor3 = accent or THEME.Purple
    topLine.BorderSizePixel = 0
    topLine.Parent = frame

    local stripe = Instance.new("Frame")
    stripe.Size = UDim2.new(0, 4, 1, -18)
...
local mappingHint = text(mappingBox, "Setiap Discord ID dapat mengatur webhook FISH dan LEAVE sendiri.", 35, 9, THEME.Muted)
...
mappingScroll.Size = UDim2.new(1, -26, 0, 246)
...
-- -----------------------------------------------------------------------------
-- 8C. FILTER TIER + FILTER KHUSUS GEMSTONE RUBY
-- -----------------------------------------------------------------------------
local filtersPage = configPage
local filterCard = card(filtersPage, 238, THEME.Orange)
text(filterCard, "⚙  CATCH FILTERS", 10, 11, THEME.Orange, true)
local filterHint = text(filterCard, "Tier tetap seperti sebelumnya. Filter weight sudah dihapus.", 35, 9, THEME.Muted)
...
refreshMappings = function()
    ...
    for index, discordId in ipairs(discordIds) do
        ...
        idLabel.Position = UDim2.fromOffset(10, 6)
        idLabel.Size = UDim2.new(1, -20, 0, 19)
        ...
        playerLabel.Position = UDim2.fromOffset(10, 26)
        playerLabel.Size = UDim2.new(1, -20, 0, 19)
        ...
        local fishButton = makeButton(
            ...
            "",
            UDim2.new(0, 10, 0, 52),
            UDim2.new(0.34, -7, 0, 27),
            THEME.Green
        )
        local leaveButton = makeButton(
            row,
            "",
            UDim2.new(0.34, 4, 0, 52),
            UDim2.new(0.34, -7, 0, 27),
            THEME.Green
        )
        local removeButton = makeButton(
            row,
            "DELETE",
            UDim2.new(0.68, 4, 0, 52),
            UDim2.new(0.32, -14, 0, 27),
            THEME.Red
        )

        local function refreshRoutingButtons()
            prefs = state.ensureMappingWebhookPrefs(discordId)

            fishButton.Text = prefs.Fish ~= false and "FISH  ON" or "FISH  OFF"
            fishButton.BackgroundColor3 = prefs.Fish ~= false and THEME.Green or THEME.Surface2
            fishButton.TextColor3 = prefs.Fish ~= false and Color3.fromRGB(8, 20, 15) or THEME.Muted

            leaveButton.Text = prefs.Leave ~= false and "LEAVE  ON" or "LEAVE  OFF"
            leaveButton.BackgroundColor3 = prefs.Leave ~= false and THEME.Green or THEME.Surface2
            leaveButton.TextColor3 = prefs.Leave ~= false and Color3.fromRGB(8, 20, 15) or THEME.Muted
        end

        refreshRoutingButtons()

        fishButton.Activated:Connect(function()
            prefs.Fish = not (prefs.Fish ~= false)
            refreshRoutingButtons()
            mappingLogStatus.TextColor3 = THEME.Orange
            mappingLogStatus.Text = "FISH diubah. Tekan CONFIG → SAVE untuk menyimpan."
            appendMonitorLog(prefs.Fish and "✅" or "⏸", "FISH " .. (prefs.Fish and "ON" or "OFF") .. ": <@" .. discordId .. ">")
        end)

        leaveButton.Activated:Connect(function()
            prefs.Leave = not (prefs.Leave ~= false)
            refreshRoutingButtons()
            mappingLogStatus.TextColor3 = THEME.Orange
            mappingLogStatus.Text = "LEAVE diubah. Tekan CONFIG → SAVE untuk menyimpan."
            appendMonitorLog(prefs.Leave and "✅" or "⏸", "LEAVE " .. (prefs.Leave and "ON" or "OFF") .. ": <@" .. discordId .. ">")
        end)
        ...
        removeButton.Activated:Connect(function()
            ...
            state.MappingWebhookPrefs[discordId] = nil
            refreshMappings()
            mappingLogStatus.TextColor3 = THEME.Orange
