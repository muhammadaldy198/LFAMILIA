...
local function manualLoad(slot)
    ...
    if data.CONFIG then
        ...
        if type(data.CONFIG.ANTI_AFK) == "boolean" then
            CONFIG.ANTI_AFK = data.CONFIG.ANTI_AFK
        end
...
local antiAFKButton = makeButton(
    homeStatus,
    "🛡  ANTI-AFK: ON",
    UDim2.new(0.60, -4, 0, 59),
    UDim2.new(0.40, -10, 0, 32),
    THEME.Green
)
...
antiAFKButton.Activated:Connect(toggleAntiAFK)
...
local function updateAntiAFKButton()
    if not antiAFKButton then return end
    antiAFKButton.Text = CONFIG.ANTI_AFK and "🛡  ANTI-AFK: ON" or "🛡  ANTI-AFK: OFF"
    antiAFKButton.BackgroundColor3 = CONFIG.ANTI_AFK and THEME.Green or THEME.Surface2
end
...
local function setAntiAFKEnabled(enabled)
    CONFIG.ANTI_AFK = enabled == true
    stopAntiAFK()

    if CONFIG.ANTI_AFK and LocalPlayer then
        local ok, connection = pcall(function()
            return LocalPlayer.Idled:Connect(function()
                local simulated = pcall(function()
                    VirtualUser:CaptureController()
                    VirtualUser:ClickButton2(Vector2.new(0, 0))
                end)

                -- Fallback untuk executor yang tidak menyediakan ClickButton2.
                ...
                appendMonitorLog("🛡", "Anti-AFK mengirim input idle")
            end)
        end)
        ...
        if ok and connection then
            ...
            appendMonitorLog("⚠️", "Anti-AFK tidak tersedia di executor ini")
        end
    end
...
local function toggleAntiAFK()
    setAntiAFKEnabled(not CONFIG.ANTI_AFK)
    local saved = manualSave(activeSlot or 1)
    if not saved then
        appendMonitorLog("ℹ️", "Anti-AFK berubah sementara; profile belum disimpan")
    end
end
...
loadBtn.Activated:Connect(function()
    ...
    if ok then
        ...
        setAntiAFKEnabled(CONFIG.ANTI_AFK)
        refreshMappings()
...
local centerMainPosition = UDim2.new(0.5, 0, 0.5, 0)
min.Activated:Connect(function()
    minimized = not minimized
    -- Panel kecil selalu dipusatkan. Posisi UI besar tidak diwariskan ke
    -- mode minimize sehingga tombol tidak tersangkut di tepi layar Android.
    main.Position = centerMainPosition
...
header.InputBegan:Connect(function(input) 
    -- Mode minimize dikunci di tengah; drag hanya aktif saat panel terbuka.
...
-- Muat slot utama otomatis jika sebelumnya pernah disimpan, lalu aktifkan
-- Anti-AFK sesuai konfigurasi. Tombol LOAD tetap tersedia untuk slot lain.
pcall(function()
